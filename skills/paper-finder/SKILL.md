---
name: paper-finder
description: "Finds and organizes ML, CV, NLP, and AI research papers based on textual descriptions and keywords. Searches across arxiv, Google Scholar, Semantic Scholar, and top venues (CVPR, ECCV, ICCV, NeurIPS, ICML, ICLR, SIGGRAPH, SIGGRAPH Asia, EMNLP, ACL, CoRL, RSS, AAAI, IJCAI, WACV, BMVC, MICCAI, KDD, WWW, NAACL, COLM, and more). Maintains a persistent memory bank, mind graph, BibTeX, and when .pipeline exists, synchronizes discovered papers into literature_bank.md and paper_bank.json. Use when the user wants to find papers, search related work, build a literature review, compare papers, generate BibTeX, or manage a research paper collection."
---
...
If the current directory contains `.pipeline/`, operate in project mode. Read
existing memory before searching:
...
In project mode, still create or update a topic folder when helpful, but always
synchronize accepted discoveries to:
...
Run 3-4 parallel searches per query:
...
2. **Record**: Add papers to memory-bank.md, update mind-graph.md, write references.bib; in project mode also update `.pipeline/memory/literature_bank.md` and `.pipeline/docs/paper_bank.json`
