...
description: summarize machine learning experiment logs in Chinese when the input includes training logs, eval results, hyperparameter changes, user notes, experiment workspace revisions, nested baseline commits, or multiple runs and the user needs a grounded experiment summary, error analysis, best configuration recap, or project-level result sync.
---

# Experiment Log Summarizer

用这个 skill 整理实验日志、参数改动、训练结果和失败记录，输出中文实验总结。重点是区分证据与猜测，并把 `experiments/` 中的分散结果同步成 `.pipeline/` 可以消费的轻量研究记录。

## 工作流

1. 先读 `.pipeline/docs/experiment_map.md`、`.pipeline/memory/experiment_ledger.md` 和 `.pipeline/docs/result_summary.md`，如果这些文件存在。
2. 把输入按实验轮次、代码路径、revision/commit、配置、结果、artifact 和备注拆开。
...
- 接收顶层 repo revision、嵌套 baseline commit、run id、artifact 路径或链接。
- 如果日志不完整，优先整理可确认事实，再列缺口。
- 如果同一实验有多次重复运行，优先总结稳定趋势，不要被单次波动误导。
- 如果结果依赖嵌套或第三方代码，必须记录代码路径和 revision/commit；缺失时标为 `missing`，不要假设。

## 输出规则

- 默认输出：
  - 本次实验目标
  - code path / revision / artifact 对应关系
  - 做了哪些改动
  - 结果变化
  - 可能原因
  - 当前最佳配置
...
- 写入 project repo 的结果必须能追溯到代码路径、revision/commit、配置或用户提供的明确证据。
